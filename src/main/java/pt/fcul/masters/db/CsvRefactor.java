...
public class CsvRefactor {
	...
	public static String mapper(String line,String market, String timeframe) {
		try { 
			...
			String[] date = split[0].replace("/", " ").replace("-", " ").trim().split(" ");
//			LocalDateTime datetime = LocalDateTime.of(Integer.parseInt(date[2]), Integer.parseInt(date[0]), Integer.parseInt(date[1]), 0, 0);
			LocalDateTime datetime = LocalDateTime.of(Integer.parseInt(date[0]), Integer.parseInt(date[1]), Integer.parseInt(date[2]), 0, 0);
//			LocalDateTime datetime = LocalDateTime.of(Integer.parseInt(date[2]), Integer.parseInt(date[1]), Integer.parseInt(date[0]), 0, 0);
			...
			System.err.println("Error evaluating line:\n"+line);
			throw e;
		}
	}
}
