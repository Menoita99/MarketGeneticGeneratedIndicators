...
public class ProfitSeekingVgpRunner {
	...
	public static void main(String[] args) {
		try {
			//			List<Market> markets= List.of(Market.AAPL,Market.BTC_USD,Market.CSCO,Market.ETH_USD,Market.FORD,Market.MSFT,Market.QCOM,Market.SPY,Market.TWTR);
			List<Market> markets= List.of(Market.INFY);
	...
	private static ProfitSeekingVGP standartConfs(Market m) {
		try {
			...
			VectorTable table = new VectorTable(m,TimeFrame.D,LocalDateTime.of(2012, 1, 1, 0, 0),21, new DynamicStepNormalizer(25*6));
			...
			ColumnUtil.addEma(table,"volumeNorm",50,21,"volEma50");
...
//							Var.of("profitPercentage", table.getColumns().size()),
							Var.of("volEma50",  table.columnIndexOf("volEma50")),
							...
							//			EphemeralConst.of(() -> Vector.random(VECTOR_SIZE).dot(Vector.of(100).sub(Vector.of(-50)))),c
